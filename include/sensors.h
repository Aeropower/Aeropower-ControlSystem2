...
static const float RB =
    1.0f / (1.0f / 470000.0f + 1.0f / 220000.0f);  // ≈149860 Ω
static const float DIV_RATIO =
    RB / (RT + RB);  // ≈ 0.5994 (Vpin = DIV_RATIO * Vsensor)
static const float UNDIVER =
    1.0f / DIV_RATIO;  // ≈ 1.668  (Vsensor = Vpin * UNDIVER)
...
const float zeroWindAdjustment =
    0.4;  // vuelve a calibrar con el “vaso” si hace falta
// const float zeroWindAdjustment =  -0.5; // negative numbers yield smaller
// wind speeds and vice versa.
...
// Helpers
static inline float adc_to_vpin(int raw) {
  return (raw * VREF) / ADC_MAX;
}  // V en el pin
static inline float vpin_to_vsrc(float vpin) {
  return vpin * UNDIVER;
}  // V reales del sensor (5 V rail)
static inline float vsrc_to_counts10(float vsrc) {
  return vsrc * (1023.0f / 5.0f);
}  // cuentas equivalentes 10 bits/5 V
