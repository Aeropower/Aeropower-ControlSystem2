...
static inline float get_wind_speed() {  // Raw es el valor leido del ADC
  // 1) Leer ADC (ESP32)
    ...
    // 2) Convertir a voltios en el pin, y luego deshacer divisor -> voltios reales del sensor (a 5 V)
    float VpinTMP = adc_to_vpin(rawTMP);
    float VpinRV  = adc_to_vpin(rawRV);

    float VsrcTMP = vpin_to_vsrc(VpinTMP);   // voltaje “real” que vería el sensor si lo midieras sin divisor
    float VsrcRV  = vpin_to_vsrc(VpinRV);

    // 3) Para fórmulas originales que usan CUENTAS de 10 bits / 5V, convertimos Vsrc a “AD units” equivalentes
    int TMP_Therm_ADunits = (int) roundf( vsrc_to_counts10(VsrcTMP) );
   int RV_Wind_ADunits   = vsrc_to_counts10(VsrcRV);   // si lo necesitas en cuentas, aquí está
    ...
    int TempCtimes100 = (0.005f * ((float)TMP_Therm_ADunits * (float)TMP_Therm_ADunits))
                    - (16.862f * (float)TMP_Therm_ADunits) + 9075.4f;

    int zeroWind_ADunits = -0.0006f * ((float)TMP_Therm_ADunits * (float)TMP_Therm_ADunits)
                       + 1.0727f * (float)TMP_Therm_ADunits + 47.172f;

    // zeroWind_volts también en voltios REALES del sensor (5V) → usa factor 5/1023
    float zeroWind_volts = (zeroWind_ADunits * (5.0f / 1023.0f)) - zeroWindAdjustment;

    // Velocidad: usa Vsrc (voltaje real del sensor), no Vpin
    float WindSpeed_MPH = powf( ((RV_Wind_Volts - zeroWind_volts) / 0.2300f), 2.7265f );
